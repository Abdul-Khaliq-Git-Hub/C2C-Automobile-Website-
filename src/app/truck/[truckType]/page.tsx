...
export default async function TruckPage({
  ...
  params: { truckType: string };
}) {
  const { truckType } = params;
  const decodedTruck = decodeURIComponent(truckType);
  // Firestore query to get car listings for the selected truckType
  const q = query(
    collection(db, "carListings"),
    where("TruckType", "==", decodedTruck)
  );
  ...
  return (
    <div>
      ...
      {carsWithImages.length > 0 ? (
        ...
        <p>No cars found for {truckType}</p>
      )}
    </div>
  );
}
